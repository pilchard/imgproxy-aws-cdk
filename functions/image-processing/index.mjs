...
const MAX_IMAGE_SIZE = Number.parseInt(process.env.maxImageSize);
...
export const handler = async (event) => {
	...
	timingLog = `${timingLog},img-transform;dur=${Number.parseInt(performance.now() - startTime)}`;
	...
	if (S3_TRANSFORMED_IMAGE_BUCKET) {
		...
		try {
			...
			timingLog = `${timingLog},img-upload;dur=${Number.parseInt(performance.now() - startTime)}`;
